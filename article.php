...
//小七修改
if($article_id !=0){
 $query = "SELECT * FROM  `ecs_article` WHERE article_id=$article_id";
$resultyi = $GLOBALS['db']->query($query);
$daohyi = $GLOBALS['db']->fetchRow($resultyi);
$daihgoyi =$daohyi['cat_id'];
 $query = "SELECT parent_id FROM  `ecs_article_cat` WHERE cat_id=$daihgoyi";
$resulter = $GLOBALS['db']->query($query);
$row_array = $GLOBALS['db']->fetchRow($resulter);
}
if(isset($article) && $article['cat_id'] > 2 and $row_array['parent_id'] !='3')
{
    $smarty->display('article.dwt', $cache_id);
}
elseif($row_array['parent_id'] == '3'){ $smarty->display('bangzhu.dwt', $cache_id);}
else
{
...
function get_article_info($article_id)
{
    ...
    if ($row !== false)
    {
		...
		/* 关键字替换 add by www.mobanyuan.com begin  */
...
/* 关键字替换 add by www.mobanyuan.com end  */
