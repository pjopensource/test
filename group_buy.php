...
if ($_REQUEST['act'] == 'list')
{
    ...
    if (!$smarty->is_cached('group_buy_list.dwt', $cache_id))
    {
        if ($count > 0)
        {
			...
			$gb_list_hot = group_buy_list_hot();
...
function group_buy_list($size, $page)
{
    ...
    $now = gmtime();
	/*新团购模板加入*/
    $sql = "SELECT b.*, IFNULL(g.goods_thumb, '') AS goods_thumb, IFNULL(g.goods_img, '') AS goods_img, g.goods_brief, g.market_price, g.shop_price, b.act_id AS group_buy_id, ".
    ...
    while ($group_buy = $GLOBALS['db']->fetchRow($res))
    {
		...
		/*新团购模板加入 时间倒计时 */
		$group_buy['gmt_end_date']     = local_date('M d, Y H:i:s',$group_buy['end_date']);
        $time = gmtime();
	   ...
	   // 当前数量
		...
		$group_buy['surplus_day']=floor(($group_buy['end_date']-time())/86400);
...
function group_buy_list_hot()
{
    ...
    $sql = "SELECT b.*, IFNULL(g.goods_thumb, '') AS goods_thumb, IFNULL(g.goods_img, '') AS goods_img, g.goods_brief, g.market_price, g.shop_price, b.act_id AS group_buy_id, ".
            ...
            "FROM " . $GLOBALS['ecs']->table('goods_activity') . " AS b " .
                "LEFT JOIN " . $GLOBALS['ecs']->table('goods') . " AS g ON b.goods_id = g.goods_id " .
            "WHERE b.act_type = '" . GAT_GROUP_BUY . "' and b.isg_new=1 " .
