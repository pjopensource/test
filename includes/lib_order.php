...
function order_goods($order_id)
{
    $sql = "SELECT rec_id, goods_id, goods_name, goods_sn, market_price, goods_number, " .
            ...
            " WHERE order_id = '$order_id'";  // 修改 by www.jfsoo.com 增加 package_attr_id 字段
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        {
            $row['package_goods_list'] = get_package_goods($row['goods_id'], $row['package_attr_id']); // 修改 by www.jfsoo.com
        }
        $goods_list[] = $row;
    }
...
function cart_goods($type = CART_GENERAL_GOODS)
{
    $sql = "SELECT rec_id, user_id, goods_id, goods_name, goods_sn, goods_number, " .
            "market_price, goods_price, goods_attr, is_real, extension_code, parent_id, is_gift, is_shipping, package_attr_id, " .
            "goods_price * goods_number AS subtotal " .
            "FROM " . $GLOBALS['ecs']->table('cart') .
            " WHERE session_id = '" . SESS_ID . "' " .
            "AND rec_type = '$type'"; //这句SQL有修改 by www.jfsoo.com 增加一个 package_attr_id
    ...
    foreach ($arr as $key => $value)
    {
        ...
        {
            $arr[$key]['package_goods_list'] = get_package_goods($value['goods_id'], $value['package_attr_id']); //修改 by www.jfsoo.com
        }
    }
...
function get_cart_goods()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
        ...
        if ($row['extension_code'] == 'package_buy')
        {
            $row['package_goods_list'] = get_package_goods($row['goods_id'], $row['package_attr_id'] ); //修改 by www.jfsoo.com 增加一个变量
        }
        $goods_list[] = $row;
    }
 ...
 * @return  boolean
 */
 /* 代码修改_start By www.jfsoo.com  提示：增加了两个参数 */
...
//    }
    ...
    $parent = array(
        ...
        'market_price'  => $package_pricea[0] ? $package_pricea[0] :  $package['market_package'], //修改 by www.jfsoo.com
        'goods_price'   => $package_pricea[1] ? $package_pricea[1] :  $package['package_price'], //修改 by www.jfsoo.com
		'package_attr_id' =>$package_attr_id, //增加 by www.jfsoo.com
        'goods_number'  => $num,
        'goods_attr'    => '',
        'goods_attr_id' => '',
    ...
    if ($num > 0)
    {
        ...
        $sql = "SELECT goods_number FROM " .$GLOBALS['ecs']->table('cart').
                ...
                " AND package_attr_id = '$package_attr_id'  AND rec_type = '" . CART_GENERAL_GOODS . "'";  //修改 by www.jfsoo.com 增加一条件
