 ...
 * @return  array
 */
function assign_cat_goods($cat_id, $num = 0, $from = 'web',$return='cat', $order_rule = '')  //zhong改  2
{
  ...
  /* zhong改上 */
	...
	*/
	$sql = 'SELECT cat_id,cat_name FROM ' . $GLOBALS['ecs']->table('category') . " WHERE parent_id = '$cat_id' order by sort_order  limit 0,4";
...
//增加一个函数
function get_promote_goods_mod($cats = '',$number='')
{
    ...
    /* 取得促销lbi的数量限制 */
    $num = get_library_number("recommend_promotion");
    $sql = 'SELECT g.goods_id, g.goods_name, g.goods_name_style, g.market_price, g.shop_price AS org_price, g.promote_price,g.goods_number, ' .
                "IFNULL(mp.user_price, g.shop_price * '$_SESSION[discount]') AS shop_price, ".
                "promote_start_date, promote_end_date, g.goods_brief, g.goods_thumb, goods_img, b.brand_name, " .
                "g.is_best, g.is_new, g.is_hot, g.is_promote, RAND() AS rnd " .
            'FROM ' . $GLOBALS['ecs']->table('goods') . ' AS g ' .
    ...
    foreach ($result AS $idx => $row)
    {
        ...
        $goods[$idx]['id']           = $row['goods_id'];
				...
				$goods[$idx]['discount']     = number_format(intval($row['promote_price'])/intval($row['market_price'])*10,1,".","");
        $goods[$idx]['name']         = $row['goods_name'];
        $goods[$idx]['brief']        = $row['goods_brief'];
        $goods[$idx]['brand_name']   = $row['brand_name'];
        $goods[$idx]['goods_style_name']   = add_style($row['goods_name'],$row['goods_name_style']);
