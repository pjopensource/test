...
E_D("replace into `ecs_mail_templates` values('15','quick_buy','1','快速购买','<p>亲爱的{\$order.consignee}，你好！ <br /><br /><br /><br />我们已经收到您于 {\$order.formated_add_time} 提交的订单，该订单编号为：{\$order.order_sn} 请记住这个编号以便日后的查询。<br /><br /><br /><br />{if \$username}<br /><br />系统自动为您创建的帐号：{\$username}<br /><br />密码：{\$password}<br /><br />{/if}<br /><br />{\$shop_name}<br /><br />{\$sent_date}</p>','1323156837','0','template');");
